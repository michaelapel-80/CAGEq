//! Stage 0 candidates that were measured and **rejected** (or kept only as comparisons) —
//! retained so `examples/warp_spike.rs` can still reproduce the decision. Nothing in
//! production calls into this module; see [`crate::matched`] for what ships and why.
//!
//! * [`constrained`] / [`relaxed`] — the thesis's general section under value/slope
//!   constraints, extended to shelves: accurate where feasible, infeasible above
//!   ~0.63·Nyquist for Q ≥ 0.5, and the relaxation toward RBJ produced wild filters.
//! * [`mz`] — impulse-invariant poles + three-point magnitude-matched numerator: worse than
//!   RBJ for shelves, failures on peaking.
//! * [`vicanek_butterworth_shelf`] / [`vicanek_shelf`] — Vicanek's published shelf and its
//!   Q-general extension with his (Butterworth-tuned) match points, before per-Q tuning.

use std::f64::consts::PI;

use crate::analog::Prototype;
use crate::matched::{vicanek_shelf_at, Failure, IDENTITY};
...
/// The general section of [`crate::matched::prescribed`] under any three [`Con`]straints, with `G0`/`G1`
...
/// table of [`crate::matched::prescribed`] gives the coefficients.
///
...
/// [`vicanek_shelf_at`] with the paper's own (Butterworth-tuned) match points.
