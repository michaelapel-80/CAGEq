...
export function Vectorscope({
  ...
  useEffect(() => {
    ...
    if (!phos.precise) console.warn("[Vectorscope] phosphor fell back to the 8-bit canvas accumulator (no half-float GPU support)");
    const [ar, ag, ab] = parseHex(getComputedStyle(cv).getPropertyValue("--accent"));
    ...
    const render = () => {
      ...
      // Retargeting crossfades from whatever was running rather than snapping (`FadingInverse` in
      // biquad.ts): an instant swap applies the *new* inverse to samples that are still the *old*
      // filter's real output, a discontinuity a snapshot recording proved isn't in the real
      ...
      // match A/B (whose preamp the inverse also removes) — or while fading out a departing
      // cascade that had either.
  ...
  const CONTROLS: { key: keyof Params; label: string; hint: string; min: number; max: number; step: number }[] = [
    ...
    { key: "beam", label: t("scope.beam"), hint: t("scope.beamHint"), min: 0.1, max: 5, step: 0.05 },
    { key: "focus", label: t("scope.focus"), hint: t("scope.focusHint"), min: 1, max: 24, step: 0.5 },
    { key: "radiusFrac", label: t("scope.scale"), hint: t("scope.scaleHint"), min: 0.3, max: 0.5, step: 0.01 },
    { key: "gridAlpha", label: t("scope.grid"), hint: t("scope.gridHint"), min: 0, max: 0.5, step: 0.02 },
